  ...
  return (
    <div>
     ...
     <button onClick={()=>scrollfn('left')}  className=' hidden md:block absolute    bg-[#baa5a56a]  left-[1rem] md:left-[3.5rem] w-[1.6rem] h-[2rem] md:w-[2rem] md:h-[6rem] rounded-sm z-index:10 hover:bg-red-200 hover:scale-125 duration-300 transform-3d  transition-transform  shadow-2xl'>&#9664;</button>
        ...
        <div ref={ref} className={`flex ml-[.1rem] mr-[.1rem]  md:mr-[3rem] md:ml-[4rem] p-2 md:p-4 gap-[.5rem]  ${ali_type==='row'?'overflow-x-auto md:gap-[6.5rem] ':'flex-wrap md:gap-[4.5rem]'}  flex-1 whitespace-nowrap no-scrollbar`}>
    
    
        {products &&
        products.map((product)=>{
          return <div key={product.id} onClick={()=>navigate(`/detail/${product.id}`)} className='flex md:w-[14rem] justify-center items-center flex-col cursor-pointer hover:scale-105 transition-transform'>
          <img src={product.main_image} className='w-[2.5rem] h-[3rem] md:w-[12rem] md:h-[12rem]'/>
         
<h2 className='text-blue-500  text-center break-words whitespace-normal text-xs md:text-lg  hover:text-black '>{product.name}</h2>

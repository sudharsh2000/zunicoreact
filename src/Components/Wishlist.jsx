...
return <div key={item.id} className='border-b-2 p-1 md:p-5 min-h-[6rem] shadow-lg rounded-xl border-gray-300 flex-row flex justify-around'>
                    ...
                    <div  className='flex w-[70%]  gap-2 md:gap-[9rem] px-2 md:px-[2rem] justify-center md:justify-around items-center'>
                      ...
                      <div className='flex flex-col justidy-start gap-1 w-[75%] md:gap-[2rem]'> 
                        <h2 onClick={()=>navigate(`/detail/${item.Product.id}`)}  className='hover:text-blue-400 text-xs md:text-xl cursor-pointer text-center font-medium'>{item.Product.name}</h2>
                        <p className='hidden md:block w-[100%] overflow-y-auto max-h-[3rem] line-clamp-4 truncate md:max-h-fit text-xs md:text-base'>{item.Product.description}

                        </p>
                      </div> 
                    </div>
                    <div className='flex w-[50%] gap-2 md:gap-4 flex-col md:flex-row  justify-center items-center'>
                      ...
                      <div className='w-[90%] md:w-[35%] flex justify-center gap-2 md:gap-3 items-center'> 
